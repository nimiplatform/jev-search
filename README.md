...
The whole thing streams. `POST /api/ask` returns newline-delimited JSON: first what the judge understood, then every engine lane on its own as soon as it has answered and its rows are scored, then a summary. On the page the first chip row is the judge's reading of the question: the words it will look for (click to change them), the time window, and the sources, lit up. Under it three plain steps tick off in order: read your question, searching these sources · N found, sorting by how well each one answers you; they fade once done. Results arrive in place without moving rows already on screen, each with the judge's on-topic percentage; rows that did not seem to match are folded under a link at the bottom. Each engine call has an 8-second cap so one slow engine cannot hold the page.
