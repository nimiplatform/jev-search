...
The whole thing streams. `POST /api/ask` returns newline-delimited JSON: first what the judge understood, then every engine lane on its own as soon as it has answered and its rows are scored, then a summary. On the page the source chips are the progress: they light up when the request has been read, breathe while their engines run, and show a count when done; one sentence says what is being looked for and where; results arrive in place without moving rows already on screen; off-topic rows are folded under a "show N off-topic results" link rather than deleted. Each engine call has an 8-second cap so one slow engine cannot hold the page.
